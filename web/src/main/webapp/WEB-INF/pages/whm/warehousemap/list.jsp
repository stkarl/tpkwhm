...
<head>
    <title><fmt:message key="whm.warehousemap.list"/></title>
    <meta name="heading" content="<fmt:message key="whm.warehousemap.list"/>"/>
</head>

<c:url var="url" value="/whm/warehousemap/list.html"/>
<c:url var="editUrl" value="/whm/warehousemap/edit.html"/>
        ...
        <div id="container-fluid data_content_box">
            <div class="row-fluid data_content">
                <div class="content-header">
                    <fmt:message key="whm.warehousemap.list"/>
                </div>
                <div class="clear"></div>
                ...
                <display:table name="items.listResult" cellspacing="0" cellpadding="0" requestURI="${url}"
                    ...
                    <display:caption><fmt:message key='whm.warehousemap.list'/></display:caption>
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="name" titleKey="label.name" style="width: 20%" >
                        ${tableList.name}
                    </display:column>
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="code" titleKey="label.name" style="width: 10%" >
                        ${tableList.code}
                    </display:column>
                    ...
                    <display:column sortable="false" headerClass="table_header_center" titleKey="label.options" style="width: 10%; text-align:center">
                        <a href="${editUrl}?pojo.warehouseMapID=${tableList.warehouseMapID}" class="icon-edit tip-top" title="<fmt:message key="label.edit"/>"></a>
                        <security:authorize ifAnyGranted="ADMIN">
                            <span class="separator"></span>
                            <a name="deleteLink" href="<c:url value='/whm/warehousemap/list.html'><c:param name="checkList" value="${tableList.warehouseMapID}"/><c:param name="crudaction" value="delete"/></c:url>" class="icon-remove tip-top" title="<fmt:message key="label.delete"/>"></a>
                        </security:authorize>
                    </display:column>
                    <display:setProperty name="paging.banner.item_name"><fmt:message key= "whm.warehousemap.name"/></display:setProperty>
                    <display:setProperty name="paging.banner.items_name"><fmt:message key= "whm.warehousemap.name"/></display:setProperty>
