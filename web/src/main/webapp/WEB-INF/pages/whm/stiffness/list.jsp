...
<head>
    <title><fmt:message key="whm.stiffness.list"/></title>
    <meta name="heading" content="<fmt:message key="whm.stiffness.list"/>"/>
</head>

<c:url var="url" value="/whm/stiffness/list.html"/>
<c:url var="editUrl" value="/whm/stiffness/edit.html"/>
<div id="content">
    <form:form commandName="items" action="${url}" method="post" id="listForm" name="listForm">
        <div id="container-fluid data_content_box">
            <div class="row-fluid data_content">
                <div class="content-header">
                    <fmt:message key="whm.stiffness.list"/>
                </div>
                <div class="clear"></div>
                    ...
                    <div class="row-fluid report-filter">
                        <table class="tbReportFilter">
                            ...
                            <tr>
                                ...
                                <td><form:input path="pojo.name" stiffness="40"/></td>
                                <td ><fmt:message key="label.code"/></td>
                                <td><form:input path="pojo.code" stiffness="40"/></td>
                            </tr>
                ...
                <display:table name="items.listResult" cellspacing="0" cellpadding="0" requestURI="${url}"
                    ...
                    <display:caption><fmt:message key='whm.stiffness.list'/></display:caption>
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="name" titleKey="label.name" style="width: 35%" >
                        ${tableList.name}
                    </display:column>
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="code" titleKey="label.code" style="width: 15%" >
                        ${tableList.code}
                    </display:column>
                    ...
                    <display:column sortable="false" headerClass="table_header_center" titleKey="label.options" style="width: 15%; text-align:center">
                        <a href="${editUrl}?pojo.stiffnessID=${tableList.stiffnessID}" class="icon-edit tip-top" title="<fmt:message key="label.edit"/>"></a>
                        <security:authorize ifAnyGranted="ADMIN">
                            <span class="separator"></span>
                            <a name="deleteLink" href="<c:url value='/whm/stiffness/list.html'><c:param name="checkList" value="${tableList.stiffnessID}"/><c:param name="crudaction" value="delete"/></c:url>" class="icon-remove tip-top" title="<fmt:message key="label.delete"/>"></a>
                        </security:authorize>

                    </display:column>
                    <display:setProperty name="paging.banner.item_name"><fmt:message key= "whm.stiffness.name"/></display:setProperty>
                    <display:setProperty name="paging.banner.items_name"><fmt:message key= "whm.stiffness.name"/></display:setProperty>
