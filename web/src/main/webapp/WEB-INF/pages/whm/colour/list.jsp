...
<head>
    <title><fmt:message key="whm.colour.list"/></title>
    <meta name="heading" content="<fmt:message key="whm.colour.list"/>"/>
</head>

<c:url var="url" value="/whm/colour/list.html"/>
<c:url var="editUrl" value="/whm/colour/edit.html"/>
<div id="content">
    <form:form commandName="items" action="${url}" method="post" id="listForm" name="listForm">
        <div id="container-fluid data_content_box">
            <div class="row-fluid data_content">
                <div class="content-header">
                    <fmt:message key="whm.colour.list"/>
                </div>
                <div class="clear"></div>
                ...
                <display:table name="items.listResult" cellspacing="0" cellpadding="0" requestURI="${url}"
                    ...
                    <display:caption><fmt:message key='whm.colour.list'/></display:caption>
                    ...
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="sign" titleKey="label.sign" style="width: 25%" >
                        ${tableList.sign}
                    </display:column>
                    <display:column sortable="false" headerClass="table_header_center" titleKey="label.options" style="width: 15%; text-align:center">
                        <a href="${editUrl}?pojo.colourID=${tableList.colourID}" class="icon-edit tip-top" title="<fmt:message key="label.edit"/>"></a>
                        <security:authorize ifAnyGranted="ADMIN">
                            <span class="separator"></span>
                            <a name="deleteLink" href="<c:url value='/whm/colour/list.html'><c:param name="checkList" value="${tableList.colourID}"/><c:param name="crudaction" value="delete"/></c:url>" class="icon-remove tip-top" title="<fmt:message key="label.delete"/>"></a>
                        </security:authorize>
                    </display:column>
                    <display:setProperty name="paging.banner.item_name"><fmt:message key= "whm.colour.name"/></display:setProperty>
                    <display:setProperty name="paging.banner.items_name"><fmt:message key= "whm.colour.name"/></display:setProperty>
