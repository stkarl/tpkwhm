...
<head>
    <title><fmt:message key="whm.import.product.by.plan.title"/></title>
    <meta name="heading" content="<fmt:message key="whm.import.product.by.plan.title"/>"/>
</head>

<c:url var="url" value="/whm/productionplan/importproductbyplan.html"/>
<c:url var="importProductUrl" value="/whm/viewbyplan/importproduct.html"/>
        ...
        <div id="container-fluid data_content_box">
            <div class="row-fluid data_content">
                <div class="content-header">
                    <fmt:message key="whm.import.product.by.plan.title"/>
                </div>
                <div class="clear"></div>
                ...
                <display:table name="items.listResult" cellspacing="0" cellpadding="0" requestURI="${url}"
                    ...
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="name" titleKey="label.description" style="width: 35%" >
                        <c:if test="${!empty mapPlanNote[tableList.productionPlanID]}">
                            Số cuộn thành phẩm: ${mapPlanNote[tableList.productionPlanID]}
                        </c:if>
                        <c:if test="${empty mapPlanNote[tableList.productionPlanID]}">
                            Chưa kê khai
                        </c:if>
                    ...
                    <display:column sortable="false" headerClass="table_header_center" titleKey="label.options" style="width: 10%; text-align:center">
                        <a href="${importProductUrl}?productionPlanID=${tableList.productionPlanID}" class="icon-eye-open tip-top" title="<fmt:message key="view.by.plan"/>"></a>
                    </display:column>
