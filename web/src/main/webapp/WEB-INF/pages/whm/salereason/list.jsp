...
<head>
    <title><fmt:message key="whm.salereason.list"/></title>
    <meta name="heading" content="<fmt:message key="whm.salereason.list"/>"/>
</head>

<c:url var="url" value="/whm/salereason/list.html"/>
<c:url var="editUrl" value="/whm/salereason/edit.html"/>
<div id="content">
    <form:form commandName="items" action="${url}" method="post" id="listForm" name="listForm">
        <div id="container-fluid data_content_box">
            <div class="row-fluid data_content">
                <div class="content-header">
                    <fmt:message key="whm.salereason.list"/>
                </div>
                <div class="clear"></div>
                ...
                <div class="report-filter">
                    <div class="row-fluid report-filter">
                        <table class="tbReportFilter">
                            ...
                            <tr>
                                <td ><fmt:message key="label.reason"/></td>
                                <td colspan="3"><form:input path="pojo.reason" size="40"/></td>
                            </tr>
                ...
                <display:table name="items.listResult" cellspacing="0" cellpadding="0" requestURI="${url}"
                    ...
                    <display:caption><fmt:message key='whm.salereason.list'/></display:caption>
                    <display:column headerClass="table_header"  escapeXml="false" sortable="true" sortName="reason" titleKey="label.reason" style="width: 50%" >
                        ${tableList.reason}
                    </display:column>
                    ...
                    <display:column sortable="false" headerClass="table_header_center" titleKey="label.options" style="width: 15%; text-align:center">
                        <a href="${editUrl}?pojo.saleReasonID=${tableList.saleReasonID}" class="icon-edit tip-top" title="<fmt:message key="label.edit"/>"></a>
                        <security:authorize ifAnyGranted="ADMIN">
                            <span class="separator"></span>
                            <a name="deleteLink" href="<c:url value='/whm/salereason/list.html'><c:param name="checkList" value="${tableList.saleReasonID}"/><c:param name="crudaction" value="delete"/></c:url>" class="icon-remove tip-top" title="<fmt:message key="label.delete"/>"></a>
                        </security:authorize>
                    </display:column>
                    <display:setProperty name="paging.banner.item_name"><fmt:message key= "whm.salereason.name"/></display:setProperty>
                    <display:setProperty name="paging.banner.items_name"><fmt:message key= "whm.salereason.name"/></display:setProperty>
