...
<html>
<head>
    <title><fmt:message key="import.material.title"/></title>
    <meta name="heading" content="<fmt:message key='import.material.title'/>"/>
</head>
<c:url var="urlForm" value="/whm/importmaterialbill/list.html"></c:url>
<c:url var="viewUrl" value="/whm/importmaterialbill/view.html"></c:url>
<c:url var="editUrl" value="/whm/importmaterialbill/edit.html"></c:url>
<body>
<div class="row-fluid data_content">
    <div class="content-header"><fmt:message key="import.material.title"/></div>
    <div class="clear"></div>
    ...
    <div class="report-filter">
        <form:form commandName="items" action="${urlForm}" id="listForm" method="post" autocomplete="off" name="listForm">
            ...
            <display:table name="items.listResult" cellspacing="0" cellpadding="0" requestURI="${formlUrl}"
                ...
                <display:caption><fmt:message key='import.material.list'/></display:caption>
                <display:column headerClass="table_header_center" sortable="false" title="STT" class="text-center" style="width: 2%;">
                    ${tableList_rowNum}
                </display:column>
                <display:column headerClass="table_header_center" titleKey="label.number" sortName="code" sortable="true" class="text-center" style="width: 10%;">
                    ${tableList.code} / ${mapBillNoInMonth[tableList.importMaterialBillID]}
                </display:column>
                ...
                <security:authorize ifAnyGranted="NHANVIENTT,QUANLYTT,LANHDAO">
                    <display:column headerClass="table_header large" sortName="totalMoney" sortable="true" titleKey="import.material.sotien" style="width: 10%;text-align:right;">
                ...
                <display:column headerClass="table_header_center" sortName="soTien" sortable="true" titleKey="label.options" style="width: 10%;text-align:center;">
                    <a href="${viewUrl}?pojo.importMaterialBillID=${tableList.importMaterialBillID}" class="icon-eye-open tip-top" title="<fmt:message key="label.view"/>"></a>
                    <c:if test="${allowEdit}">
                        <span class="separator"></span>
                        <a href="${editUrl}?pojo.importMaterialBillID=${tableList.importMaterialBillID}" class="icon-edit tip-top" title="<fmt:message key="label.edit"/>"></a>
                    </c:if>
                    <c:if test="${allowDelete}">
                        <span class="separator"></span>
                        <a name="deleteLink" id="${tableList.importMaterialBillID}" onclick="warningDelete(this)" href="#" class="icon-remove tip-top" title="<fmt:message key="label.delete"/>"></a>
                    </c:if>
                </display:column>
                <display:setProperty name="paging.banner.item_name" value="Phiếu nhập vật tư"/>
                <display:setProperty name="paging.banner.items_name" value="Phiếu nhập vật tư"/>
