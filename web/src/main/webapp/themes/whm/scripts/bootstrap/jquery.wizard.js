...
(function($){
    $.widget("ui.formwizard", {
        ...
        _init: function() {
            ...
            this.firstStep = this.options.currentStep;//this.steps.eq(0).attr("id");
            this.activatedSteps = new Array();
            this.isLastStep = false;
            this.previousStep = undefined;
            this.currentStep = this.options.currentStep;//this.steps.eq(0).attr("id");

            var mIndex = this.options.currentStep.split('-')[2];
            if(mIndex != null && mIndex != ""){
                mIndex = parseInt(mIndex);
                if(mIndex > 1){
                    for(var i = 1; i < mIndex + 1; i++){
                        this.activatedSteps.push('form-wizard-' + i);
                    }
                }
            }
            ...
            this.nextButton = $("#next").click(function(){
                return wizard._next();
            });
            ...
            this.backButton = $("#back").click(function(){
                wizard._back();return false;
            });
...
//this.nextButton.val(this.options.textSubmit);
        ...
        _show : function(step){
            ...
            /* active or disable buttons replace and add new*/
            var questionIdOfStep = parseInt($("#"+step).attr("title"));
            var rowData = $.parseJSON($.cookie("CUSTOM_TEST"));
            for(var index = 0; index < rowData.length; index++){
                if(rowData[index].questionId == questionIdOfStep){
                    $("#replace").attr("disabled", "disabled");
                    $("#addAsNew").attr("disabled", "disabled");
                    break;
                }else{
                    $("#replace").removeAttr("disabled");
                    $("#addAsNew").removeAttr("disabled");
                }
            }
